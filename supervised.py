...
def main():
    ...
    if cfg['criterion']['name'] == 'CELoss':
        ...
        criterion = nn.CrossEntropyLoss(ignore_index=255, weight=torch.FloatTensor([1, 10])).cuda(local_rank)
    ...
    if rank == 0:
        logger.info('raw:  labeled_data_num: {:}     val_data_num: {:}'.format(
                    len(set(trainset)), len(set(valset.ids))))
        logger.info('乘后： labeled_data_num: {:}     val_data_num: {:}'.format(
                    len(trainset), len(valset.ids)))
